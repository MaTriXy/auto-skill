...
**Why:** Skills CLI (`npx skills find`) provides superior search experience with interactive fzf-style UI, better filtering, and direct installation. Auto-Skill now focuses on **skill generation** rather than duplicating distribution capabilities.
...
This clarifies Auto-Skill's role as a **skill factory** (generation + publishing) rather than duplicating Skills CLI's distribution capabilities.
...
### Removed Files
Deprecated modules replaced by v5.0 architecture:

- `src/core/skillssh-client.ts` - Replaced by `external-skill-loader.ts`
- `src/core/providers/skillssh-provider.ts` - Replaced by unified discovery system
- `src/cli/commands/discover.ts:searchCommand` - Function removed, use Skills CLI

### Deprecated Modules
- `src/core/unified-suggester.ts` - External skill integration deprecated
  - Now returns empty array for external suggestions
  - Use `createExternalSkillLoader()` + `createProactiveDiscovery()` instead

### Code Removals
- Removed `createSkillsShClient` export from `src/index.ts`
- Removed `createSkillsShProvider` export from `src/core/providers/index.ts`
- Removed `SearchOptions` interface from CLI commands
...
- **Skyll:** [https://github.com/assafelovic/skyll](https://github.com/assafelovic/skyll)
